...
import {BaseMap, DocArr, Doctor, DoctorResponseData, HospitalWordResponseData, WorkData} from "@/api/hospital/type.ts";

defineOptions({name: 'RegisterStep1'})
...
let workData = reactive<HospitalWordResponseData>({
  total: 0,
  bookingScheduleList: [],
  baseMap: {} as BaseMap
})

let workTime = ref<WorkData>({} as WorkData)
...
const fetchWorkData = async () => {
  let result = await reqHospitalWork(pageNo.value,
      pageSize.value,
      $route.query.code as string,
      $route.query.depcode as string)
  if(result.code == 200) {
    workData.total = result.data.total
    workData.bookingScheduleList = result.data.bookingScheduleList
    workData.baseMap = result.data.baseMap
    //存储第一天日期的数据
...
const changeTime = (item: WorkData) => {
  //存储用户选择那一天的数据
  workTime.value = item
  //再发一次获取医生排班的数据
  getDoctorWorkData()
...
const getDoctorWorkData = async () => {
  ...
  //获取排班医生的数据
  let result: DoctorResponseData = await reqHospitalDoctor(code, depCode, workDate);
  if (result.code == 200) {
...
<div class="wrap">
  <div class="top">
    <div class="hospitalName">{{workData.baseMap.hosname}}</div>
    <div class="line"></div>
  ...
  <div class="center">
    <h1 class="title-time">{{workData.baseMap.workDateString}}</h1>
    <div class="container">
      <div class="item"
           :class="{active:item.status==-1||item.availableNumber==-1||item.availableNumber==0, cur: item.workDate == workTime.workDate,}"
           v-for="item in workData.bookingScheduleList"
           :key="item.workDate"
           @click="changeTime(item)">
        <div class="header">{{item.workDate}} {{item.dayOfWeek}}</div>
        <div class="bottom">
          <span v-if="item.status==-1">停止挂号</span>
          <span v-if="(item.status==0)&&(item.availableNumber==-1)">约满了</span>
          <span v-if="(item.status==0)&&(item.availableNumber>-1)">有号({{item.availableNumber}})</span>
  ...
  <div class="bot">
    ...
    <div class="doctor" v-else>
      ...
      <div class="morning">
        ...
        <div class="doctor-info" v-for="doctor in morningArr" :key="doctor.id">
          ...
          <div class="right">
            ...
            <el-button :disabled="doctor.availableNumber < 1" type="primary" size="default" @click="goStep2(doctor)">{{
                doctor.availableNumber>0?doctor.availableNumber:0
              }}</el-button>
          </div>
        </div>
      </div>
      ...
      <div class="morning">
        ...
        <!--每一个医生的信息-->
        <div class="doctor-info" v-for="doctor in afterArr" :key="doctor.id">
          <!-- 展示医生的名字|技能 -->
          <div class="left">
            <div class="info">
              <span>{{ doctor.title }}</span>
